...
object SpecifiedDirection {
  ...
  private[chisel3] def specifiedDirection[T <: IsBindable](
    source: => T
...
/**
  ...
  */
object Input {
  def apply[T <: IsBindable](source: => T): T = {
...
/** This forms the roots of the type system for Chisel's data types.
  ...
  */
trait IsBindable extends HasId with NamedComponent {
  ...
  // Must clone an IsBindable if any of the following are true:
  // * It has a binding
  ...
  /** The minimum (aka "oldest") id that is part of this IsBindable
    *
  ...
  /** Internal API; Chisel users should look at chisel3.chiselTypeOf(...).
    *
    * cloneType must be defined for any Chisel object extending IsBindable.
  ...
  /** Internal API; Chisel users should look at chisel3.chiselTypeOf(...).
    *
    * Returns a copy of this IsBindable type, with bindings (if any) removed.
    * Directionality data and probe information is still preserved.
    */
  private[chisel3] def cloneTypeFull: this.type

  /** Adds this `IsBindable` to its parents _ids if it should be added */
...
/** This forms the root of the type system for wire data types. The data value
  ...
  * @define coll data
  */
abstract class Data extends IsBindable with SourceInfoDoc {
