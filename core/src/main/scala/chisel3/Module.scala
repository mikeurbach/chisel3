...
package experimental {
  ...
  abstract class BaseModule extends HasId with IsInstantiable {
    ...
    private[chisel3] def getModulePortsAndLocators: Seq[(IsBindable, SourceInfo)] = {
      require(_closed, "Can't get ports before module close")
      _ports.toSeq
    }
    ...
    private[chisel3] def findPort(name: String): Option[IsBindable] =
    ...
    /**
      ...
      */
    private[chisel3] def getChiselPorts(implicit si: SourceInfo): Seq[(String, IsBindable)] = {
      require(_closed, "Can't get ports before module close")
    ...
    /** Chisel2 code didn't require the IO(...) wrapper and would assign a Chisel type directly to
      ...
      */
    protected def _bindIoInPlace(iodef: IsBindable)(implicit sourceInfo: SourceInfo): Unit = {

      // Assign any signals (Chisel or chisel3) with Unspecified/Flipped directions to Output/Input.
      // This is only required for Data, not all IsBindables in general.
      iodef match {
    ...
    /**
      ...
      *
      * Also registers an IsBindable as a port, also performing bindings. Cannot be called once ports are
      * requested (so that all calls to ports will return the same information).
      * Internal API.
      *
      * TODO(twigg): Specifically walk the IsBindable definition to call out which nodes
      * are problematic.
      */
    protected def IO[T <: IsBindable](iodef: => T)(implicit sourceInfo: SourceInfo): T = {
