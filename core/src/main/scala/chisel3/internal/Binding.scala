...
object requireIsHardware {
  def apply(node: IsBindable, msg: String = ""): Unit = chisel3.experimental.requireIsHardware.apply(node, msg)
}
...
object requireIsChiselType {
  def apply(node: IsBindable, msg: String = ""): Unit = chisel3.experimental.requireIsChiselType.apply(node, msg)
}
